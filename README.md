...
This is a **test-automation playground**, not a production app. There is **no password change, reset, or forgot-password feature** — by design. QA engineers practice login/logout/role-switching via the Session lab instead. The mock auth accepts any email + any password ≥ 4 chars.

### Real-world booking flow (not dummy)
The check-in flow mirrors real airline apps:
1. **Book** → search → select flight → seat map → passenger details → payment → PNR
2. **Check in** → multi-step dialog:
   - **Step 1 — Seat selection**: interactive seat map (available/premium/taken)
   - **Step 2 — Addons**: food (veg/non-veg/vegan/kids), baggage, comfort, digital
   - **Step 3 — Confirm**: review seat + addons + price
   - **Step 4 — Done**: boarding pass PDF auto-downloads, status → `checked_in`
3. **After check-in**: "Boarding pass" button replaces "Check in" — re-download PDF anytime
4. **Cancel booking**: `window.confirm()` dialog → status → `cancelled`
5. **Request refund**: dialog with reason textarea + file upload (supporting docs) → status → `cancelled`

### PDF downloads (not HTML)
Both invoice and boarding pass are generated as **real PDF files** using jsPDF — entirely client-side, no server round-trip. Playwright tests capture them via `page.waitForEvent('download')` and assert on `.suggestedFilename()` (`.pdf` extension).
